...
const injectCode = start => `const myScript = document.createElement('script');
    myScript.text = "window.onload = () => console.log(Date.now() - ${start});";
    document.getElementsByTagName('head')[0].appendChild(myScript);`; // assign the listener function to a variable so we can remove it later


const devToolsListener = (message, sender, sendResponse) => {
  window.alert(message); // Inject a content script into the identified tab
